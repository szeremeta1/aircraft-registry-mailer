...
const formatModelName = (manufacturer: string, model: string): string => {
...
export const processFiles = async (directory: string): Promise<Aircraft[]> => {
    const potentialAircraft: Aircraft[] = [];
    ...
    try {
        ...
        records.slice(5).forEach((record: any[], index: number) => {
            try {
                processedCount++;
                ...
                if (!Object.keys(MANUFACTURER_MAPPINGS).some(mfr => 
                    manufacturer.toUpperCase().includes(mfr))) {
